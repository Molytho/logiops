...
namespace {
    constexpr const char virtual_input_name[] = "LogiOps Virtual Input";
    constexpr const char default_config[]     = "/etc/logid.cfg";

    struct CmdlineOptions {
        std::string config_file {default_config};
    };

    // maybe TODO: rework with boost
    enum class Option { None, Verbose, Config, Help, Version };

    // maybe TODO: rework with boost
    CmdlineOptions readCliOptions(const int argc, char **argv) {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (argv[i][0] == '-') {
                ...
                case 'h': // Help
                    ...
                    break;
                default:
                    logPrintf(WARN, "%s is not a valid option, ignoring.", argv[i]);
                }
                ...
                case Option::Verbose: {
                    ...
                    try {
                        ...
                        if (argv[i][0] == '-') {
                            ...
                            printf(
                                "Valid verbosity levels are: Debug, Info, "
...
int main(int argc, char **argv) {
    CmdlineOptions options = readCliOptions(argc, argv);
